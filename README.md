...
There is no live model response in the shipped runtime. GPT-5.6 and Codex were used to build and validate the product, as documented separately below; they are not presented as an online response generated by the Spatial Index. A future optional **LabSpace AI API** may provide model-driven intent selection and explanations over the existing index and validator tools; until implemented, it is described only as API-ready future architecture.
