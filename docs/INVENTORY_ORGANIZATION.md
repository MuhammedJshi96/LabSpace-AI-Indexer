...
**Public deployment caution:** Render's public judge mode currently uses temporary, isolated server-memory sessions. A restart, inactivity shutdown or deployment can discard online edits. Export the current online project before deploying; never replace it with a local project or starter snapshot. The Storage UI does not change that persistence policy.
