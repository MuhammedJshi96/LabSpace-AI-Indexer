...
This folder defines a deterministic competition eval set for the thirteen public LabSpace WebMCP tools. It is an expected-call suite, not a claim that a language model is embedded in LabSpace or that every model will choose an identical optional inspection step.

## What the automated check proves

`tests/unit/webmcp-evals.test.ts` verifies that every case has a unique identifier, uses only the published thirteen-tool surface, declares forbidden mutations, and requires human approval whenever a move, room blueprint, or inventory proposal is staged. The underlying action tests separately verify canonical reads, catalog and location search, exact record focus, deterministic geometry, polygon room plans, inventory plans, reversible staging, approval, cancellation, persistence, bounded output, and controlled errors.
...
2. Inspect `await document.modelContext.getTools()` and confirm the exact thirteen tools in `cases.json` are registered once.
