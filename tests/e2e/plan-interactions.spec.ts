...
test("placed assets drag without jumping to the plan origin", async ({ page }) => {
  ...
  const xInput = page.getByRole("spinbutton", { name: "X mm", exact: true });
  const yInput = page.getByRole("spinbutton", { name: "Y mm", exact: true });
...
test("dragging one wall side keeps connected room corners joined", async ({ page, request }) => {
  ...
  expect(east.wall.start.x).toBe(north.wall.end.x);
  expect(east.wall.start.y).toBe(north.wall.end.y);
  expect(west.wall.end.x).toBe(north.wall.start.x);
